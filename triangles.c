...
void drawTriangle(struct Triangle *triangle, int fill) {
  ...
  Matrix_MultiplyPoint(matWorld, triangle->p[0], &triTransformed.p[0]);
  Matrix_MultiplyPoint(matWorld, triangle->p[1], &triTransformed.p[1]);
  Matrix_MultiplyPoint(matWorld, triangle->p[2], &triTransformed.p[2]);
  ...
  if (Point_DotProduct(normal, vCameraRay) < 0.0f) {
    ...
    int nClipped = 0;
    struct Triangle clipped[2];
    nClipped = Triangle_ClipAgainstPlane((struct Point){0, 0, 0.1, 1},
                                         (struct Point){0, 0, 1, 1}, &triViewed,
                                         &clipped[0], &clipped[1]);
    ...
    for (int n = 0; n < nClipped; n++) {
      ...
      // copyTriangle(&triProjected, &triToDraw[place++]);
